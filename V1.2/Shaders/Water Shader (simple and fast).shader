...
uvnh.xy += half2(_WaterLocalUvNX * -0.7, _WaterLocalUvNZ * -1.3);

half h = tex2D(_ParallaxMap, uvnh * _ParallaxScale).r;
half2 offset = ParallaxOffset(h, _Parallax + _WaterParallax, IN.viewDir);
...
uvnd.xy += half2(_WaterLocalUvNX * -0.7, _WaterLocalUvNZ * -1.3);

fixed4 tex = tex2D(_MainTex, uv * 1.2);
tex *= tex2D(_Detail, uvd * 0.75);
tex *= 2;
...
normal *= tex2D(_BumpMapDetail, uvnh * 5) * 0.9;
normal *= tex2D(_BumpMapDetail, uvnh * 10) * 0.8;
